...
(function() {
    ...
    function send(level: any, message: any, meta: any, source: any) {
        const payload = {
    ...
    function broadcastLocal(payload: any) {
        try {
    ...
    function removeFromLocalBackup(count: any) {
        try {
    ...
    function detectCategory(source: any, level: any) {
    ...
    function formatError(event: any) {
    ...
    function cleanStack(stack: any) {
        if (!stack) return '';
        return stack.split('\n').filter((l: any) => !l.includes('node_modules')).slice(0, 5).join('\n');
    }
    ...
    window.FantroveConsole = {
        log: (msg: any, meta: any) => send('log', msg, meta, 'API'),
        info: (msg: any, meta: any) => send('info', msg, meta, 'API'),
        warn: (msg: any, meta: any) => send('warn', msg, meta, 'API'),
        error: (msg: any, meta: any) => send('error', msg, meta, 'API'),
        debug: (msg: any, meta: any) => send('debug', msg, meta, 'API'),
        success: (msg: any, meta: any) => send('success', msg, meta, 'API'),
        captureException: (err: any, ctx: any) => send('error', err.message, { stack: err.stack, context: ctx }, 'ManualCapture'),
