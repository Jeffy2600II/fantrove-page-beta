...
class FantroveConsolePro {
    ...
    initStarted: any;
    isCapturing: any;
    isCloudConnected: any;
    isInitialized: any;
    isLoadingHistory: any;
    isOnline: any;
    isSyncing: any;
    lastActiveUsersUpdate: any;
    lastPollTime: any;
    loadedTimeRange: any;
    logs: any;
    pendingLogs: any;
    ...
    async connectToCloud() {
        ...
        try {
            ...
            // @ts-expect-error TS(2339): Property 'json' does not exist on type 'unknown'.
    ...
    async loadGlobalLogs() {
        try {
            ...
            if (result.logs.length > 0) {
                // เรียงจากใหม่ -> เก่า
                this.logs = result.logs.sort((a: any, b: any) => b.timestamp - a.timestamp);
                this.logs = this.logs.slice(0, 500);

                this.loadedTimeRange.newest = Math.max(...this.logs.map((l: any) => l.timestamp));
                this.loadedTimeRange.oldest = Math.min(...this.logs.map((l: any) => l.timestamp));
    ...
    async loadOlderLogs() {
        ...
        try {
            ...
            if (result.logs.length > 0) {
                const existingIds = new Set(this.logs.map((l: any) => l.id));
                const newLogs = result.logs.filter((l: any) => !existingIds.has(l.id));
        ...
        try {
            ...
            if (result.logs.length > 0) {
                ...
                if (newLogs.length > 0) {
                    ...
                    const hasError = newLogs.some((l: any) => l.level === 'error');
                    const hasWarn = newLogs.some((l: any) => l.level === 'warn');
    ...
    async fetchLogs(params = {}) {
        const { 
            ...
            after = null, 
            // @ts-expect-error TS(2339): Property 'fromStart' does not exist on type '{}'.
            fromStart = false,
        ...
        // @ts-expect-error TS(2339): Property 'ok' does not exist on type 'unknown'.
        if (!res.ok) {
            // @ts-expect-error TS(2339): Property 'status' does not exist on type 'unknown'... Remove this comment to see the full error message
        ...
        return {
            logs: (data.logs || []).map((log: any) => this.normalizeLog(log)),
    ...
    fetchWithTimeout(url: any, options: any, timeoutMs: any) {
    ...
    async addLog(log: any, saveToCloud = true, skipStorage = false) {
        try {
    ...
    shouldSkipStorage(log: any) {
        if (log.category !== 'system' && log.source !== 'System') return false;
        const message = log.message || '';
        return this.skipStoragePatterns.some((pattern: any) => pattern.test(message));
    }

    shouldDisplay(log: any) {
    ...
    renderLog(log: any, animate = true) {
    ...
    refreshDisplay() {
        ...
        const toShow = this.logs.filter((log: any) => this.shouldDisplay(log));
        ...
        if (toShow.length === 0) {
            ...
            toShow.forEach((log: any) => this.renderLog(log, false));
        }
    }
            ...
            try {
                ...
                if (response.status >= 500) {
                    this.captureError({
                        ...
                        // @ts-expect-error TS(2339): Property 'url' does not exist on type 'Request | U... Remove this comment to see the full error message
    ...
    handleConnectionFailure(errorMessage: any) {
    ...
    setSyncStatus(syncing: any, message = '') {
    ...
    log(msg: any, meta: any, skipStorage = false) { 
        this.addLog({ 
            level: 'log', 
            message: msg, 
            source: 'Console', 
    ...
    info(msg: any, meta: any, skipStorage = false) { 
        this.addLog({ 
            level: 'info', 
            message: msg, 
            source: 'Console', 
    ...
    warn(msg: any, meta: any, skipStorage = false) { 
        this.addLog({ 
            level: 'warn', 
            message: msg, 
            source: 'Console', 
    ...
    error(msg: any, meta: any, skipStorage = false) { 
        this.addLog({ 
            level: 'error', 
            message: msg, 
            source: 'Console', 
    ...
    debug(msg: any, meta: any, skipStorage = false) { 
        this.addLog({ 
            level: 'debug', 
            message: msg, 
            source: 'Console', 
    ...
    system(msg: any, meta: any, skipStorage = false) { 
        this.addLog({ 
            level: 'info', 
            message: msg, 
            source: 'System', 
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => {
        // @ts-expect-error TS(2339): Property 'consolePro' does not exist on type 'Wind... Remove this comment to see the full error message
...
// @ts-expect-error TS(2339): Property 'FantroveConsole' does not exist on type ... Remove this comment to see the full error message
window.FantroveConsole = {
    // @ts-expect-error TS(2339): Property 'consolePro' does not exist on type 'Wind... Remove this comment to see the full error message
    log: (m: any, meta: any) => window.consolePro?.log(m, meta),
    // @ts-expect-error TS(2339): Property 'consolePro' does not exist on type 'Wind... Remove this comment to see the full error message
    info: (m: any, meta: any) => window.consolePro?.info(m, meta),
    // @ts-expect-error TS(2339): Property 'consolePro' does not exist on type 'Wind... Remove this comment to see the full error message
    warn: (m: any, meta: any) => window.consolePro?.warn(m, meta),
    // @ts-expect-error TS(2339): Property 'consolePro' does not exist on type 'Wind... Remove this comment to see the full error message
    error: (m: any, meta: any) => window.consolePro?.error(m, meta),
    // @ts-expect-error TS(2339): Property 'consolePro' does not exist on type 'Wind... Remove this comment to see the full error message
    debug: (m: any, meta: any) => window.consolePro?.debug(m, meta),
    // @ts-expect-error TS(2339): Property 'consolePro' does not exist on type 'Wind... Remove this comment to see the full error message
    success: (m: any, meta: any) => window.consolePro?.addLog({ 
        level: 'success', 
        message: m, 
        source: 'API', 
        category: 'api', 
