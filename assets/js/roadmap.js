...
(function () {
        ...
        try {
            return a.current_stage !== b.current_stage ||
                ...
                a.stages.some((s, i) => {
                    ...
                    if (!t)
                        return true;
                    if (s.version !== t.version || s.stage_number !== t.stage_number)
                        return true;
                    if (s.features.length !== t.features.length)
                        return true;
                    for (let j = 0; j < s.features.length; j++) {
                        ...
                        for (let k = 0; k < k1.length; k++) {
                            if (s.features[j].feature[k1[k]] !== t.features[j].feature[k1[k]])
                                return true;
                        }
                    }
                    return false;
    ...
    (async function boot() {
        ...
        getFeatureDataCache().then(cached => {
            if (cached)
                displayFeatures(cached.current_stage, cached.stages);
        });
    ...
    window.addEventListener("languageChange", function () {
        if (inMemoryCache)
            displayFeatures(inMemoryCache.current_stage, inMemoryCache.stages);
        else
            getFeatureDataCache().then(data => {
                if (data)
