...
(function (global) {
  ...
  function defaultNormalizeText(s: any) {
    if (!s && s !== 0) return '';
    s = String(s).toLowerCase().trim();
    try { s = s.normalize('NFKD').replace(/[\u0300-\u036f]/g, ''); } catch (e) {}
    s = s.replace(/[\u200B-\u200D\uFEFF]/g, '');
    s = s.replace(/[\u2018\u2019\u201A\u201B\u2032\u2035]/g, "'").replace(/[\u201C\u201D\u201E\u201F\u2033\u2036]/g, '"');
    s = s.replace(/[\uFF01-\uFF5E]/g, (ch: any) => String.fromCharCode(ch.charCodeAt(0) - 0xFEE0));
    s = s.replace(/[^\p{L}\p{N}\s]+/gu, ' ');
    s = s.replace(/\s+/g, ' ').trim();
    return s;
  }

  function pickLang(obj: any, langs: any){
  ...
  function buildImmediateDocs(data: any) {
    const docs: any = [];
    const keywords: any = [];
  ...
  function flattenDataToDocs(data: any, normalizeFn: any) {
    const docs: any = [];
    const keywords: any = [];
    if (!data || !Array.isArray(data.type)) return { docs, keywords };
  ...
  const SearchEngine = (function(){
    let _data: any = null;
    let _docs: any = [];            // immediate docs (un-normalized combined strings)
    let _keywords: any = [];        // immediate keywords (normalized via toLowerCase for prefix match)
    let _fuse: any = null;         // Fuse instance (built async)
    ...
    async function init(data: any, options: any) {
      options = options || {};
    ...
    function immediateSearch(qRaw: any, typeFilter: any, limit: any) {
    ...
    function querySuggestions(rawQuery: any, maxCount: any) {
      maxCount = maxCount || 8;
    ...
    function search(qRaw: any, typeFilter: any) {
      const q = String(qRaw || '').trim();
      if (!q) return { results: [], keywords: generateAllKeywords() };
    ...
    return {
      init: function(data: any, options: any) { return init(data, options); },
      generateAllKeywords: function() { return generateAllKeywords(); },
      querySuggestions: function(q: any, maxCount: any) { return querySuggestions(q, maxCount); },
      search: function(q: any, typeFilter: any) { return search(q, typeFilter); },
      _internals: {
