...
const _indexEngine = {
    ...
    build(assembled) {
        this.reset();
        if (!assembled || !Array.isArray(assembled.type))
            return;
        for (const typeObj of assembled.type) {
            ...
            for (const cat of (typeObj.category || [])) {
                ...
                for (const item of items) {
                    ...
                    if (item.api)
                        this._apiIndex.set(item.api, enriched);
                    if (item.text)
                        this._textIndex.set(item.text, enriched);
    ...
    searchByName(query) {
        if (!this._nameIndex || !query)
            return [];
        const q = query.toLowerCase().trim();
        const results = new Map();
        const exact = this._nameIndex.get(q);
        if (exact)
            exact.forEach((i) => results.set(i.api || i.text || i, i));
        for (const [key, items] of this._nameIndex) {
            if (key.includes(q))
                items.forEach((i) => results.set(i.api || i.text || i, i));
        }
...
const _eventBus = {
    ...
    off(event, fn) { const b = this._listeners.get(event); if (b)
        b.delete(fn); },
    emit(event, payload) {
        ...
        b.forEach((fn) => { try {
            ...
            console.warn('ConDataService event error', e);
        } });
    }
};
// =========================================================
...
const _loader = {
    ...
    async loadTopIndex() {
        ...
        if (!ConDataRegistry.validate.topIndex(raw))
            throw new Error('con-data index.json: invalid structure');
        this._topIndex = raw;
        return raw;
    },
    async assemble() {
        if (this._assembledDb)
            return this._assembledDb;
        if (this._assemblePromise)
            return this._assemblePromise;
        this._assemblePromise = (async () => {
            ...
            const typeResults = await Promise.all(topIndex.categories.map(async (catEntry) => {
                try {
                    const raw = await _fetcher.fetch(ConDataRegistry.resolvePath(catEntry.file));
                    if (!ConDataRegistry.validate.typeIndex(raw))
                        return null;
            ...
            const typeObjs = [];
            await Promise.all(typeResults.filter(Boolean).map(async ({ id: typeId, name: typeName, typeData }) => {
                const loadedCats = await Promise.all((typeData.categories || []).map(async (catEntry) => {
                    try {
                        ...
                        if (!ConDataRegistry.validate.dataFile(raw))
                            return null;
...
const ConDataService = {
    ...
    async getCategories(typeId) {
        if (!typeId)
            throw new Error('getCategories: typeId is required');
        const db = await _loader.assemble();
        const typeObj = (db.type || []).find((t) => t.id === typeId);
        if (!typeObj)
            throw new Error(`getCategories: type "${typeId}" not found`);
    ...
    async getCategoryMeta(typeId, categoryId) {
        ...
        if (!cat)
            throw new Error(`getCategoryMeta: not found ${typeId}/${categoryId}`);
    ...
    async getItems(typeId, categoryId) {
        if (!typeId || !categoryId)
            throw new Error('getItems: typeId and categoryId required');
        await _loader.assemble();
        ...
        if (!typeObj)
            throw new Error(`getItems: type "${typeId}" not found`);
        const cat = (typeObj.category || []).find((c) => c.id === categoryId);
        if (!cat)
            throw new Error(`getItems: category "${categoryId}" not found`);
        return cat.data || [];
    },
    async getAllItems(typeId = null) {
        await _loader.assemble();
        if (!typeId)
            return _indexEngine.getAllItems();
        const db = _loader._assembledDb;
        const results = [];
    ...
    async findByApiBatch(apiCodes) {
        if (!Array.isArray(apiCodes) || !apiCodes.length)
            return [];
        await _loader.assemble();
    ...
    async getFormatted(format = 'assembled', options = {}) {
        ...
        switch (format) {
            ...
            case 'api-map': {
                const out = {};
                if (this._apiIndex) {
                    for (const [api, item] of _indexEngine._apiIndex)
                        out[api] = item;
                }
                else {
                    for (const t of (db.type || []))
            ...
            case 'types-only': return (db.type || []).map((t) => ({
                id: t.id,
                name: t.name
            }));
    ...
    async filter(fn, typeId = null) {
        if (typeof fn !== 'function')
            throw new Error('filter: fn must be a function');
        const all = await this.getAllItems(typeId);
        return all.filter(fn);
    },
    async transform(fn) {
        if (typeof fn !== 'function')
            throw new Error('transform: fn must be a function');
        const db = await _loader.assemble();
        return fn(db);
    },
    // -------------------------------------------------------
        ...
        switch (action) {
            ...
            case 'filter':
                ...
                if (typeof params.fn !== 'function')
                    throw new Error('request filter: fn required');
                // @ts-expect-error TS(2339): Property 'fn' does not exist on type '{}'.
                return this.filter(params.fn, params.typeId || null);
            case 'transform':
                // @ts-expect-error TS(2339): Property 'fn' does not exist on type '{}'.
                if (typeof params.fn !== 'function')
                    throw new Error('request transform: fn required');
                // @ts-expect-error TS(2339): Property 'fn' does not exist on type '{}'.
