...
(function (global) {
    ...
    // @ts-expect-error TS(2552): Cannot find name 'scheduler'. Did you mean 'Schedu... Remove this comment to see the full error message
    ...
    // @ts-expect-error TS(2339): Property 'scheduling' does not exist on type 'Navi... Remove this comment to see the full error message
    ...
    const Scheduler = {
        task(fn, priority = 'background', signal = null) {
            if (HAS_SCHED) {
                const o = { priority };
                // @ts-expect-error TS(2339): Property 'signal' does not exist on type '{ priori... Remove this comment to see the full error message
                if (signal)
                    o.signal = signal;
                // @ts-expect-error TS(2552): Cannot find name 'scheduler'. Did you mean 'Schedu... Remove this comment to see the full error message
            ...
            return new Promise((res, rej) => {
                ...
                if (priority === 'user-blocking')
                    ...
                    requestIdleCallback(run, { timeout: priority === 'background' ? 4000 : 600 });
                else
                    (_mc ? _mcYield().then(run) : setTimeout(run, 0));
            });
        },
        yield() {
            // @ts-expect-error TS(2552): Cannot find name 'scheduler'. Did you mean 'Schedu... Remove this comment to see the full error message
        ...
        isInputPending() {
            // @ts-expect-error TS(2339): Property 'scheduling' does not exist on type 'Navi... Remove this comment to see the full error message
            try {
                return HAS_IPP && !!navigator.scheduling.isInputPending();
            }
            catch {
                return false;
            }
    ...
    class Pool {
        ...
        clear() { this._free.forEach((el) => { try {
            el.remove();
        }
        catch { } }); this._free = []; }
    }
  ...
  try{self.postMessage({id:id,html:items.map(function(it){return buildCard(it,lang,label);})});}
    ...
    const WorkerRenderer = (() => {
        ...
        return {
            render(items, lang, label) {
                const w = _boot();
                if (!w)
                    return Promise.resolve(items.map((it) => buildCardHTML(it, lang, label)));
                return new Promise((res, rej) => {
                    const id = _id++;
                    const timer = setTimeout(() => { _pend.delete(id); res(items.map((it) => buildCardHTML(it, lang, label))); }, TIMEOUT_MS);
                    _pend.set(id, { res, rej, timer });
                    try {
                        w.postMessage({ id, items, lang, label });
                    }
                    catch (e) {
                        _pend.delete(id);
                        clearTimeout(timer);
                        res(items.map((it) => buildCardHTML(it, lang, label)));
                    }
                });
            },
    ...
    class FlatList {
        ...
        constructor(opts = {}) {
            ...
            // @ts-expect-error TS(2339): Property 'onCopy' does not exist on type '{}'.
        ...
        _attachDelegate() {
            if (this._delegated || !this.host || !this.onCopy)
                return;
            ...
            this.host.addEventListener('click', (e) => {
                ...
                if (text)
                    try {
                        this.onCopy(decodeURIComponent(text));
                    }
                    catch { }
            });
        }
        clear() {
            ...
            if (this.host) {
                this.host.replaceChildren ? this.host.replaceChildren() : (this.host.innerHTML = '');
            }
        }
    ...
    class VList {
        ...
        renderItem;
        constructor(opts = {}) {
            // @ts-expect-error TS(2339): Property 'container' does not exist on type '{}'.
            ...
            // @ts-expect-error TS(2339): Property 'renderItem' does not exist on type '{}'.
            ...
            // @ts-expect-error TS(2339): Property 'fixedHeight' does not exist on type '{}'... Remove this comment to see the full error message
            this.fixedHeight = !!opts.fixedHeight;
            // @ts-expect-error TS(2339): Property 'overscan' does not exist on type '{}'.
            this.overscanPx = opts.overscan || (IS_LOW_END ? 280 : 480);
            // @ts-expect-error TS(2339): Property 'itemClass' does not exist on type '{}'.
            this.itemClass = opts.itemClass || 'vl-item';
            // @ts-expect-error TS(2339): Property 'onMount' does not exist on type '{}'.
            this.onMount = opts.onMount || null;
            // @ts-expect-error TS(2339): Property 'onRecycle' does not exist on type '{}'.
            this.onRecycle = opts.onRecycle || null;
            // @ts-expect-error TS(2339): Property 'poolMax' does not exist on type '{}'.
                ...
                if (!this.fixedHeight) {
                    this._ro = new ResizeObserver(entries => {
                        ...
                        for (const e of entries) {
                            ...
                            const idx = parseInt(e.target.dataset.vidx, 10);
                            if (isNaN(idx))
                                continue;
        ...
        _render() {
            if (this._destroyed || !this._box || !this.items.length)
                return;
            ...
            for (let i = si; i <= ei; i++) {
                if (this._vis.has(i) || !this.items[i])
                    continue;
        ...
        scrollTo(i) { if (!this._off)
            return; const t = this._off[i] ?? 0; this.container === window ? window.scrollTo({ top: t, behavior: 'smooth' }) : this.container.scrollTo({ top: t, behavior: 'smooth' }); }
        jumpTo(i) { if (!this._off)
            return; const t = this._off[i] ?? 0; this.container === window ? window.scrollTo(0, t) : (this.container.scrollTop = t); }
        invalidate() { if (this.container === window)
            this._boxTopCache = null; this._schedule(); }
        destroy() {
    ...
    class VGrid {
        ...
        constructor(opts = {}) {
            ...
            // @ts-expect-error TS(2339): Property 'renderItem' does not exist on type '{}'.
            this.renderItem = opts.renderItem || (() => '');
            // @ts-expect-error TS(2339): Property 'cellW' does not exist on type '{}'.
            this.cellW = opts.cellW || 166;
            // @ts-expect-error TS(2339): Property 'cellH' does not exist on type '{}'.
        ...
        _measure() {
            ...
            if (this._box)
                this._box.style.height = (Math.ceil(this.items.length / this._cols) * this.cellH) + 'px';
        }
        _scrollTop() { return this.container === window ? (window.pageYOffset || 0) : (this.container.scrollTop || 0); }
        _viewportH() { return this.container === window ? window.innerHeight : (this.container.clientHeight || window.innerHeight); }
        _getBoxTop() { let top = 0, el = this._box; while (el && el !== this.container) {
            top += el.offsetTop;
            el = el.offsetParent;
        } return top; }
        _bindEvents() {
            ...
            this.container.addEventListener('scroll', this._onScroll, { passive: true });
            if (HAS_RO) {
                this._ro = new ResizeObserver(() => { this._measure(); this._schedule(); });
        ...
        _schedule() {
            if (this._raf || this._destroyed)
                return;
            this._raf = requestAnimationFrame(() => { this._raf = null; if (!this._destroyed)
                this._render(); });
        }
        _render() {
            if (this._destroyed || !this._box || !this.items.length)
            ...
            for (const [idx] of this._vis)
                if (idx < fi || idx > li)
                    toRec.push(idx);
            for (const idx of toRec) {
                this._pool.release(this._vis.get(idx));
                this._vis.delete(idx);
            }
            const frag = document.createDocumentFragment();
            for (let i = fi; i <= li; i++) {
        ...
        update(items) { this.items = items || []; for (const [, el] of this._vis)
            this._pool.release(el); this._vis.clear(); this._measure(); this._schedule(); }
    ...
    /* ═══════════════════════════════════════════════════════════
       PUBLIC FACADE
    ═══════════════════════════════════════════════════════════ */
    // @ts-expect-error TS(2339): Property 'RenderEngine' does not exist on type 'Wi... Remove this comment to see the full error message
    global.RenderEngine = {
        Scheduler,
        Pool,
        FlatList,
    ...
    if (typeof module !== 'undefined' && module.exports)
        module.exports = global.RenderEngine;
})(window);
