...
(function () {
  ...
  const LanguageService = {
    ...
    setLang(lang: any) { try { localStorage.setItem(CONFIG.STORAGE.langKey, lang); } catch {} },
    t(key: any) {
  ...
  const DOMService = {
    get: (id: any) => document.getElementById(id),
    query: (sel: any) => document.querySelector(sel),
    queryAll: (sel: any) => document.querySelectorAll(sel),
    create(tag: any, id: any, cls: any, styles: any) {
    ...
    remove(el: any) { try { el?.parentNode?.removeChild(el); } catch {} },
    setStyles(el: any, s: any) { if (el) try { Object.assign(el.style, s); } catch {} },
    setHTML(el: any, h: any) { if (el) el.innerHTML = h; },
    setAttr(el: any, k: any, v: any) { if (el) el.setAttribute(k, v); },
    addClass: (el: any, c: any) => el?.classList?.add(c),
    removeClass: (el: any, c: any) => el?.classList?.remove(c),
    on(el: any, ev: any, fn: any, opts: any) { if (el && fn) el.addEventListener(ev, fn, opts); },
    off(el: any, ev: any, fn: any) { if (el && fn) el.removeEventListener(ev, fn); },
  };

  const StringService = {
    escapeHtml: (s: any) => String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'),
    encodeUrl: (s: any) => encodeURIComponent(s),
    decodeUrl(s: any) { try { return decodeURIComponent(s); } catch { return s; } },
  };

  const StorageService = {
    getHistory() { try { return JSON.parse(sessionStorage.getItem(CONFIG.STORAGE.historyKey)||'[]'); } catch { return []; } },
    addSearchToHistory(state: any) {
      try {
  ...
  const VirtualScrollEngine = {
    ...
    mount(viewport: any, host: any, items: any, renderFn: any, lang: any) {
      this.destroy();
  ...
  const RenderingService = {
    renderResultItem(item: any, lang: any) {
      try {
    ...
    extractResultCategories(results: any) {
      try {
    ...
    renderResults(results: any, showSuggestionsIfNoResult = false) {
      try {
        ...
        const filtered = State.selectedCategory !== 'all'
          ? results.filter((r: any) => ((r.category?.name?.[lang]||r.category?.name?.en)||'') === State.selectedCategory)
          : results;
        ...
        if (State.overlayOpen && State.scrollableContent) {
          VirtualScrollEngine.mount(
            ...
            (item: any, l: any) => this.renderResultItem(item, l), lang
          );
        } else {
    ...
    _batchRender(items: any, container: any, lang: any) {
      try {
        const tpl = document.createElement('template');
        tpl.innerHTML = items.map((item: any) => this.renderResultItem(item, lang)).join('');
        container.appendChild(tpl.content);
        this._attachCopyHandler(container);
      } catch (e) { console.error('_batchRender failed', e); }
    },

    _attachCopyHandler(container: any) {
  ...
  const FilterService = {
    ...
    setupCategoryFilter(cats: any, selected = 'all') {
      try {
  ...
  const SuggestionService = {
    handleKeydown(ev: any, container: any) {
      try {
    ...
    renderQuerySuggestions(query: any) {
      try {
  ...
  const OverlayService = {
    open() {
      try {
        ...
        // ── Suggestion handlers ────────────────────────────
        Handlers.suggestionKeydown = (ev: any) => SuggestionService.handleKeydown(ev, sg);
        Handlers.suggestionClick   = (ev: any) => SuggestionService.handleClick(ev);
        // @ts-expect-error TS(2554): Expected 4 arguments, but got 3.
  ...
  // =========================================================
  const SearchService = {
    doSearch(e: any, preventPush: any, options: any) {
      try {
        e?.preventDefault?.();
        options = options || {};
    ...
    doSearchFromURL(q: any, type: any, category: any, retryCount: any) {
  ...
  // DATA LOADER
  // =========================================================
  function _waitForConDataService(ms: any) {
  ...
  function loadData() {
    return _waitForConDataService(CONFIG.TIMING.conDataServiceWaitMs).then(svc => {
      ...
      if (svc) return svc.getAssembled().catch((err: any) => {
  ...
  function initializeSearchEngine() {
    try {
      ...
      loadData().then(data => {
        ...
        return initFn(State.apiData, {}).catch((e: any) => console.error('SearchEngine.init failed', e));
      ...
      if (form) {
        // @ts-expect-error TS(2554): Expected 3 arguments, but got 0.
        Handlers.formSubmit = (e: any) => { e.preventDefault(); SearchService.doSearch(); UIService.closeKB(); };
        // @ts-expect-error TS(2554): Expected 4 arguments, but got 3.
      ...
      if (inp) {
        // @ts-expect-error TS(2554): Expected 3 arguments, but got 0.
        const kd = (e: any) => { if (e.key==='Enter') { e.preventDefault(); SearchService.doSearch(); UIService.closeKB(); } };
  ...
  function _restoreUIState(st: any) {
    try {
      State.suppressHistoryPush = true;
      const inp = DOMService.get(CONFIG.DOM.searchInputId);
      // @ts-expect-error TS(2339): Property 'value' does not exist on type 'HTMLEleme... Remove this comment to see the full error message
  ...
  Object.assign(window.__searchUI, {
    ...
    // @ts-expect-error TS(2339): Property 'SearchEngine' does not exist on type 'Wi... Remove this comment to see the full error message
    querySuggestions:            (q: any) => window.SearchEngine?.querySuggestions?.(q, CONFIG.RENDER.suggestionMax) || [],
