...
(function () {
    ...
    // @ts-expect-error TS(2339): Property '__fantroveFooterInjected' does not exist... Remove this comment to see the full error message
    if (window.__fantroveFooterInjected)
        return;
    // @ts-expect-error TS(2339): Property '__fantroveFooterInjected' does not exist... Remove this comment to see the full error message
...
`;
    ...
    function gatherContentCandidates() {
        return Array.from(document.body.children).filter(el => {
            ...
            if (['script', 'link', 'style', 'meta', 'noscript', 'template'].includes(tag))
                return false;
            if (tag === 'footer' && el.classList.contains('footer-minimal'))
                return false;
            return true;
        });
    }
    ...
    function ensureSiteRoot() {
        try {
            if (document.querySelector('.site-root'))
    ...
    function placeFooterNode(footerNode, siteRoot) {
        try {
            ...
            if (footerNode && footerNode.parentNode !== document.body)
                document.body.appendChild(footerNode);
            return footerNode;
        }
    }
    ...
    function injectFooterHTML(footerHTML) {
        ...
        if (!existing) {
            ...
            if (!footerEl) {
                ...
                if (!footerEl.classList.contains('footer-minimal'))
                    footerEl.classList.add('footer-minimal');
                if (!footerEl.getAttribute('role'))
                    footerEl.setAttribute('role', 'contentinfo');
            }
        }
    ...
    function fetchFooterTemplate() {
        return fetch(FOOTER_TEMPLATE_PATH, { cache: 'force-cache' }).then(r => {
            if (!r.ok)
                return Promise.reject(new Error('Footer template not found'));
            return r.text();
        });
    }
    /* -------------------------
    ...
    // Debug hook (optional)
    // @ts-expect-error TS(2339): Property '__fantroveFooterDebug' does not exist on... Remove this comment to see the full error message
    try {
        window.__fantroveFooterDebug = { ensureSiteRoot, injectFallbackStyle, removeFallbackStyle };
    }
    catch (e) { }
})();
