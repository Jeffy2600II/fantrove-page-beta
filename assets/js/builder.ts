...
async function loadContentFile(path: any) {
  try {
...
function renderPreview() {
  ...
  groupedData[selectedCategory].forEach((item: any, idx: any) => {
    // BUTTON
    ...
    if (item.group && item.group.type === 'card' && Array.isArray(item.group.items)) {
      item.group.items.forEach((card: any, cidx: any) => {
    ...
    if (item.group && item.group.type === 'button' && Array.isArray(item.group.items)) {
      item.group.items.forEach((btnItem: any, bidx: any) => {
...
function openModal(contentHtml: any, onClose: any) {
...
// Modal alert (UX)
function showModalAlert(msg: any) {
  const overlay = document.getElementById('modal-overlay');
...
function openAddButtonDialog(category: any) {
  // Modal Card
...
function renderModalFormFields(div: any, type: any, category: any) {
  div.innerHTML = '';
  if (type === 'api') {
    div.innerHTML += `
...
function showInlineEditForm(type: any, cat: any, idx: any, item: any, subIdx = null, subItem = null) {
  const modalCard = document.createElement('div');
  modalCard.className = 'modal-card';
  let isApi, obj: any;
  if (type === 'button') {
...
function renderModalEditFields(div: any, type: any, obj: any, cat: any) {
  div.innerHTML = '';
  if (type === 'api') {
    div.innerHTML += `
      <label for="api">API (emoji/symbol code)</label>
