...
class LanguageManager {
    ...
    getEnSource() {
        if (this.languagesConfig?.en?.enSource === "json")
            return "json";
        return "html";
    }
    // ==================== LANGUAGE CHANGE ====================
    ...
    async prepareAllButtonTexts() {
        ...
        if (!this.languageButton || !this.languagesConfig)
            return;
        // Clear existing
        ...
        for (let i = 0; i < newNodes.length; i++) {
            ...
            if (newNode && newNode.__slotMarker) {
                ...
                if (slotEl) {
                    ...
                    const span = document.createElement('span');
                    if (newNode.name)
                        span.setAttribute('data-translate-slot', newNode.name);
                    else
                        span.setAttribute('data-translate-slot', 'slot');
                    if (currentOld)
                        el.insertBefore(span, currentOld);
                    else
                        el.appendChild(span);
            ...
            if (newNode && newNode.__svgMarker) {
                ...
                if (svgRef) {
                    ...
                    if (newNode.__predicted) {
                        if (el.firstChild)
                            el.insertBefore(createdSvg, el.firstChild);
                        else
                            el.appendChild(createdSvg);
                        ...
                        if (currentOld)
                            el.insertBefore(createdSvg, currentOld);
                        else
                            el.appendChild(createdSvg);
                ...
                if (currentOld.nodeType === 1 && newNode.nodeType === 1) {
                    try {
                        if (currentOld.tagName === newNode.tagName) {
                            while (currentOld.firstChild)
                                currentOld.removeChild(currentOld.firstChild);
                            ...
                            newAttrs.forEach(a => {
                                try {
                                    currentOld.setAttribute(a.name, a.value);
                                }
                                catch (e) { }
                            });
                            oldAttrs.forEach(a => {
                                if (!newNode.hasAttribute(a.name)) {
                                    try {
                                        currentOld.removeAttribute(a.name);
                                    }
                                    catch (e) { }
                                }
                            });
