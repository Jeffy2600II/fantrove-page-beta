        ...
        return new Promise((resolve, reject) => {
            ...
            s.onload = () => {
                ...
                if (global.Fuse)
                    ...
                    reject(new Error('Fuse loaded but global.Fuse not available'));
            };
                ...
                for (let x = 0; x < items.length; x++) {
                    ...
                    if (item.name && typeof item.name === 'object') {
                        for (const lg of langs)
                            if (item.name[lg])
                                parts.push(String(item.name[lg]));
                    }
                    else if (item.name)
                        parts.push(String(item.name));
                    if (item.api)
                        parts.push(String(item.api));
                    if (item.text)
                        parts.push(String(item.text));
                    for (const lg of langs) {
                        if (typeNames[lg])
                            parts.push(String(typeNames[lg]));
                        if (catNames[lg])
                            parts.push(String(catNames[lg]));
                    }
            ...
            for (let j = 0; j < cats.length; j++) {
                ...
                for (let x = 0; x < items.length; x++) {
                    ...
                    if (item.name && typeof item.name === 'object') {
                        for (const lg of langs) {
                            if (item.name[lg])
                                combinedParts.push(String(item.name[lg]));
                        }
                    }
                    else if (item.name)
                        combinedParts.push(String(item.name));
                    for (const k in item) {
                        if (/_name$/.test(k) && typeof item[k] === 'object') {
                            for (const lg of langs)
                                if (item[k][lg])
                                    combinedParts.push(String(item[k][lg]));
                        }
                    }
                    if (item.api)
                        combinedParts.push(String(item.api));
                    if (item.text)
                        combinedParts.push(String(item.text));
                    for (const lg of langs) {
                        if (typeNames[lg])
                            combinedParts.push(String(typeNames[lg]));
                        if (catNames[lg])
                            combinedParts.push(String(catNames[lg]));
                    }
                    ...
                    const kw = (doc.name || '') || (doc.api || '');
                    if (kw)
                        keywords.push({ raw: kw, normalized: normalizeFn ? normalizeFn(kw) : String(kw).toLowerCase(), docId: doc.id, item: item, itemName: doc.name, typeObj: typeObj, typeName: doc.typeKey, catName: doc.categoryKey });
                }
            }
        }
        return { docs, keywords };
        ...
        function immediateSearch(qRaw, typeFilter, limit) {
            ...
            for (let i = 0; i < _docs.length && results.length < limit; i++) {
                ...
                if (typeFilter && typeFilter !== 'all') {
                    if ((d.typeKey || '').toLowerCase() !== String(typeFilter || '').toLowerCase())
                        continue;
                }
        ...
        function querySuggestions(rawQuery, maxCount) {
            ...
            if (_fuse && q.length >= 1) {
                try {
                    ...
                    console.error('Fuse search for suggestions failed', e);
                }
            }
            else {
            ...
            if (_fuse) {
                try {
                    ...
                    for (let i = 0; i < fuseResults.length; i++) {
                        ...
                        const doc = r.item || r;
                        if (typeFilter && typeFilter !== 'all') {
                            if ((doc.typeKey || '').toLowerCase() !== String(typeFilter || '').toLowerCase())
                                continue;
                        }
                        results.push({
                            typeObj: doc.typeObj,
