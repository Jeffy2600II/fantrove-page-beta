document.addEventListener('DOMContentLoaded', function () {
    ...
    function updateTheme() {
        try {
            ...
            if (tsw)
                tsw.innerHTML = `<i class="fas fa-${isDarkMode ? 'sun' : 'moon'}"></i>`;
        }
        catch (e) { }
    }
    updateTheme();
    ...
    window.editItem = function (type, categoryId, index) {
        try {
            ...
            const langFields = supportedLanguages.map(lang => `<div class="form-group">
    ...
    window.saveEdit = function () {
        try {
            ...
            if (!validateInput(newText, newApi))
                return;
        ...
        try {
            ...
            Object.entries(apiData).forEach(([type, data]) => {
                ...
                data.category.forEach(category => {
                    if (selectedCategoryId !== 'all' && (category.id || category.name) !== selectedCategoryId)
                        return;
                    if (!category.data)
                        category.data = [];
    ...
    function removeDragCardPreview() {
        if (dragState.cardEl) {
            try {
                document.body.removeChild(dragState.cardEl);
            }
            catch (e) { }
    ...
    function getItemObj(type, categoryId, index) {
        try {
            ...
            if (cat && cat.data && cat.data[index])
                return cat.data[index];
            return {};
        }
        catch (e) {
            return {};
    ...
    function finalizeDrop(x, y) {
        try {
            ...
            if (dragState.dropTargetType && dragState.dropTargetCategoryId != null && dragState.dropTargetIndex != null) {
                if (dragState.dragType === dragState.dropTargetType &&
                    dragState.dragCategoryId === dragState.dropTargetCategoryId &&
                    dragState.dragIndex === dragState.dropTargetIndex)
                    return;
                moveItem(dragState.dragType, dragState.dragCategoryId, dragState.dragIndex, dragState.dropTargetType, dragState.dropTargetCategoryId, dragState.dropTargetIndex);
                updateUI();
            }
        }
        catch (e) {
    ...
    function cleanupDragState() {
        ...
        if (dragState.dragItem)
            dragState.dragItem.classList.remove('drag-source');
    ...
    document.getElementById('addNewItem').addEventListener('click', function () {
        try {
            ...
            showToast('เกิดข้อผิดพลาดขณะเพิ่มข้อมูล', 'error');
        }
    });
    ...
    document.getElementById('confirmAddCategoryLang').onclick = function () {
        try {
            ...
            showToast('เกิดข้อผิดพลาดขณะเพิ่มหมวดหมู่', 'error');
        }
    };
