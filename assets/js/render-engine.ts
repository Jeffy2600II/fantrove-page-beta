...
;(function (global) {
  ...
  let _mcRes: any = null, _mc: any = null;
  ...
  ═══════════════════════════════════════════════════════════ */
  const Scheduler = {
    task(fn: any, priority = 'background', signal = null) {
      if (HAS_SCHED) {
        const o = { priority };
    ...
    frame: (fn: any) => requestAnimationFrame(fn),
    idle(fn: any, t = 2000) { HAS_RIC ? requestIdleCallback(fn, { timeout: t }) : setTimeout(fn, 16); },
    async chunked(items: any, fn: any, size = IS_LOW_END ? 10 : 40, signal = null) {
  ...
  class Pool {
    ...
    clear() { this._free.forEach((el: any) => { try { el.remove(); } catch {} }); this._free = []; }
  }
  ...
  function _enc(s: any) { try { return encodeURIComponent(s); } catch { return s; } }

  function buildCardHTML(item: any, lang: any, copyLabel: any) {
...
self.onmessage=function(e){
  ...
  const WorkerRenderer = (() => {
    ...
    return {
      render(items: any, lang: any, label: any) {
        const w = _boot();
        if (!w) return Promise.resolve(items.map((it: any) => buildCardHTML(it, lang, label)));
        return new Promise((res, rej) => {
          const id    = _id++;
          const timer = setTimeout(() => { _pend.delete(id); res(items.map((it: any) => buildCardHTML(it, lang, label))); }, TIMEOUT_MS);
          _pend.set(id, { res, rej, timer });
          try { w.postMessage({ id, items, lang, label }); }
          catch (e) { _pend.delete(id); clearTimeout(timer); res(items.map((it: any) => buildCardHTML(it, lang, label))); }
        });
      },
      renderSync(items: any, lang: any, label: any) { return items.map((it: any) => buildCardHTML(it, lang, label)); },
      terminate() {
  ...
  class FlatList {
    ...
    _attachDelegate() {
      ...
      this.host.addEventListener('click', (e: any) => {
  ...
  class VList {
    ...
    scrollTo(i: any) { if (!this._off) return; const t = this._off[i]??0; this.container===window?window.scrollTo({top:t,behavior:'smooth'}):this.container.scrollTo({top:t,behavior:'smooth'}); }
    jumpTo(i: any)   { if (!this._off) return; const t = this._off[i]??0; this.container===window?window.scrollTo(0,t):(this.container.scrollTop=t); }
  ...
  class VGrid {
    ...
    update(items: any) { this.items=items||[]; for(const[,el]of this._vis)this._pool.release(el); this._vis.clear(); this._measure(); this._schedule(); }
  ...
  global.RenderEngine = {
    ...
    createFlatList: (opts: any) => new FlatList(opts),
    createVList   : (opts: any) => new VList(opts),
    createVGrid   : (opts: any) => new VGrid(opts),
