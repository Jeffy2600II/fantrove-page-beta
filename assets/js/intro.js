...
class IntroTemplateManager {
    ...
    async _preloadTemplate() {
        if (this._preloadStarted || !this.shouldShowIntro())
            return;
     ...
     */
    show() {
        if (!this.container || !this.shouldShowIntro())
            return;
    ...
    hide() {
        ...
        requestAnimationFrame(() => {
            ...
            const duration = Math.max(parseFloat(computed.transitionDuration) || 0, parseFloat(computed.animationDuration) || 0) * 1000;
            if (duration < 10) {
...
class IntroSystem {
    ...
    saveState() {
        ...
        safeSession.set(SYSTEM_CONFIG.introExpirationKey, (Date.now() + SYSTEM_CONFIG.introDuration).toString());
    }
    /**
