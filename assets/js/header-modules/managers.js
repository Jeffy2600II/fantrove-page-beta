...
export const scrollManager = {
    ...
    _Z: 999,
    _injectStyles() {
        if (document.getElementById('_hdr_sticky_css'))
            return;
...
#sub-nav.fx .hj{border-color:rgba(0,0,0,0);background:transparent;}`;
    ...
    _tick() {
        ...
        if (top <= 0 && !this._fixed) {
            sn.classList.add('fx');
            this._fixed = true;
        }
        else if (top > 0 && this._fixed) {
            sn.classList.remove('fx');
            this._fixed = false;
        }
    },
    init() {
...
export const subNavManager = {
    ensureSubNavContainer() {
        ...
        if (!sn) {
            ...
            if (h?.nextSibling)
                h.parentNode.insertBefore(sn, h.nextSibling);
            else
...
export const buttonManager = {
    ...
    async loadConfig() {
        ...
        try {
            (window._headerV2_router || window._headerV2_navigationManager)?.updateButtonStates?.();
        }
        catch (_) { }
    },
    ...
    async _handleInitialUrl(url, def) {
        try {
            if (!url || url === '?') {
                if (def)
                    await this.triggerMainButtonClick(def.button);
                return;
            }
            ...
            if (md.config.subButtons?.length) {
                if (sub)
                    await this._handleInitialSub(md.config, main, sub);
                else
                    await this._handleDefaultSub(md.config, main);
    ...
    async triggerMainButtonClick(btn) {
        ...
        try {
            ...
            console.error('triggerMainButtonClick', e);
        }
    },
    async triggerSubButtonClick(btn) {
        ...
        try {
            ...
            console.error('triggerSubButtonClick', e);
        }
    },
    async renderSubButtons(subBtns, mainUrl, lang) {
        ...
        ctr.innerHTML = '';
        const p = new URLSearchParams(window.location.search.startsWith('?') ? window.location.search : `?${window.location.search}`);
        ...
        if (needDef && defBtn)
            setTimeout(() => { try {
                this.triggerSubButtonClick(defBtn);
            }
            catch (_) { } }, 0);
    },
...
export const navigationManager = {
    ...
    updateButtonStates(url) {
        try {
            ...
            // @ts-expect-error TS(2339): Property '_headerV2_elements' does not exist on ty... Remove this comment to see the full error message
            const el = window._headerV2_elements;
            el?.navList?.querySelectorAll('button').forEach((b) => b.classList.toggle('active', b.getAttribute('data-url') === main));
            el?.subButtonsContainer?.querySelectorAll('button').forEach((b) => b.classList.toggle('active', b.getAttribute('data-url') === `${main}-${sub}`));
            // @ts-expect-error TS(2339): Property '_headerV2_router' does not exist on type... Remove this comment to see the full error message
