...
try {
    const ro = new ResizeObserver(e => { for (const r of e)
        _vp.w = r.contentRect.width; });
...
function _cvsc(el) {
    if (!('oncontentvisibilityautostatechange' in el))
        return;
    el.addEventListener('contentvisibilityautostatechange', (ev) => {
        if (ev.skipped)
            _offscreen.add(el);
        else
            _offscreen.delete(el);
    }, { passive: true });
}
function _hidePage() {
...
export const contentManager = {
    ...
    async renderContent(data) {
        ...
        const renderBatch = async (from, bsz) => {
            if (signal.aborted || sess !== this._sess)
                return 0;
            ...
            for (let i = from; i < to; i++) {
                if (items[i]?.jsonFile && !items[i]._fetched)
                    jsonSlots.push(i);
            }
            if (jsonSlots.length) {
            ...
            for (let i = from; i < limit; i++) {
                ...
                try {
                    ...
                    if (grp)
                        await this._renderGroup(inner, grp);
                    else
                        await this._renderSingle(inner, item);
                }
                catch (e) {
                    console.error('render item', e);
                }
        ...
        if (!sentinel) {
            ...
            sentinel.id = this._SID;
        }
        ctr.appendChild(sentinel);
        this._io = new IntersectionObserver(entries => {
            if (signal.aborted || sess !== this._sess || this._busy)
                return;
    ...
    _mkCtr(item) {
        ...
        if (item.group?.containerClass)
            d.classList.add(item.group.containerClass);
        return d;
    },
    async _renderGroup(ctr, grp) {
        ...
        if (grp.categoryId) {
            ...
            if (header)
                ctr.appendChild(this._mkHeader(header));
            const frag = document.createDocumentFragment();
        ...
        if (Array.isArray(grp.items)) {
            if (grp.header)
                ctr.appendChild(this._mkHeader(grp.header));
            const frag = document.createDocumentFragment();
    ...
    _mkHeader(cfg) {
        ...
        if (cfg.className)
            wrap.classList.add(cfg.className);
        ...
        if (!wrap._ll) {
            ...
            window.addEventListener('languageChange', ev => {
                ...
                if (ht && cfg.title)
                    ht.textContent = _txt(cfg.title, nl);
                const hd = wrap.querySelector('.group-header-description');
                if (hd && cfg.description)
                    hd.textContent = _txt(cfg.description, nl);
            }, { passive: true });
        }
        return wrap;
    },
    ...
    async _mkCard(cfg) {
        ...
        if (cfg.className)
            card.classList.add(cfg.className);
        return card;
    },
    updateCardsLanguage(lang) {
        document.querySelectorAll('.card').forEach(c => {
            ...
            if (t) {
                const v = t.dataset[`title${lang.toUpperCase()}`];
                if (v)
                    t.textContent = v;
            }
            // @ts-expect-error TS(2339): Property 'dataset' does not exist on type 'Element... Remove this comment to see the full error message
            ...
            if (d) {
                const v = d.dataset[`desc${lang.toUpperCase()}`];
                if (v)
                    d.textContent = v;
            }
        });
