...
const dataManager = {
    ...
    async _processFetchQueue() {
        if (this._queueProcessing || this._fetchQueue.length === 0)
            return;
    ...
    clearCache() {
        ...
        try {
            ConDataService.invalidateCache();
        }
        catch (e) { }
    },
    // =========================================================
    ...
    async _performFetch(url, options = {}) {
        ...
        try {
            ...
            if (!window._headerV2_utils.isOnline())
            ...
            const response = await fetch(url, {
                ...
                // @ts-expect-error TS(2339): Property 'cache' does not exist on type '{}'.
    ...
    async _warmup() {
        ...
        this._warmupPromise = new Promise(resolve => {
            ...
            if ('requestIdleCallback' in window)
                requestIdleCallback(doWarmup, { timeout: 2000 });
            else
                setTimeout(doWarmup, 1200);
        });
        return this._warmupPromise;
    },
    // =========================================================
    ...
    async _loadTopLevelIndex() {
        if (this._topLevelIndex)
            return this._topLevelIndex;
        if (this._topLevelIndexPromise)
    ...
    async _loadCategoryIndex(category) {
        if (this._categoryIndexes.has(category))
            return this._categoryIndexes.get(category);
    ...
    async _loadSubcategoryFile(category, subcat) {
        ...
        if (this._subcategoryCache.has(cacheKey))
            return this._subcategoryCache.get(cacheKey);
        try {
