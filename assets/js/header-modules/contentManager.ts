...
function _bg(fn: any) {
  // @ts-expect-error TS(2304): Cannot find name 'scheduler'.
...
class _Bits {
  _cnt: any;
  b: any;
  constructor(n: any){ this.b=new Uint32Array(Math.ceil((n||256)/32)); this._cnt=0; }
  has(i: any){ return !!(this.b[i>>5]&(1<<(i&31))); }
  add(i: any){ if(this.has(i))return; this.b[i>>5]|=(1<<(i&31)); this._cnt++; }
...
const _pool={
  _s:new Map(), CAP:48,
  get(t: any){ const a=this._s.get(t); return a?.length?a.pop():null; },
  put(t: any,n: any){
    if(!n)return;
...
// ─── Helpers ──────────────────────────────────────────────
const _txt=(v: any,l: any)=>!v?'':typeof v==='object'?(v[l]||v.en||''):String(v);
const _esc=(s: any) => String(s).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;');

const _offscreen=new WeakSet();
function _cvsc(el: any){
  if(!('oncontentvisibilityautostatechange' in el))return;
  el.addEventListener('contentvisibilityautostatechange',(ev: any) => {
...
export const contentManager = {
  ...
  async renderContent(data: any){
    ...
    // ────────────────────────────────────────────────────
    const renderBatch=async(from: any,bsz: any)=>{
  ...
  _isCard(item: any){return item.type==='card'||item.group?.type==='card'||(!!(item.image)&&!item.api);},
  ...
  async _renderGroup(ctr: any,grp: any){
    // @ts-expect-error TS(2339): Property '_headerV2_data_manager' does not exist o... Remove this comment to see the full error message
  ...
  async _renderSingle(ctr: any,item: any){
  ...
  _mkHeader(cfg: any){
    ...
    // @ts-expect-error TS(2339): Property '_ll' does not exist on type 'HTMLDivElem... Remove this comment to see the full error message
    if(!wrap._ll){
      // @ts-expect-error TS(2339): Property '_ll' does not exist on type 'HTMLDivElem... Remove this comment to see the full error message
      wrap._ll=true;
  ...
  createContainer(item: any){return this._mkCtr(item);},
  async createButton(cfg: any){return this._mkBtn(cfg);},
  async createCard(cfg: any){return this._mkCard(cfg);},
  async renderGroupItems(ctr: any,grp: any){return this._renderGroup(ctr,grp);},
  async renderSingleItem(ctr: any,item: any){return this._renderSingle(ctr,item);},
};
