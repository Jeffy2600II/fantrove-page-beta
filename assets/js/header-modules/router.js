...
const router = {
    ...
    normalizeUrl(input) {
        ...
        if (hasSub)
            return sub ? `?type=${main}__&page=${sub}` : `?type=${main}__`;
        return `?type=${main}`;
    },
    ...
    async validateUrl(url) {
        try {
            ...
            if (!mainBtn)
                return false;
            if (sub)
                return !!(mainBtn.subButtons?.some((sb) => sb.url === sub || sb.jsonFile === sub));
            return true;
        }
        catch (_) {
            return false;
    ...
    async getDefaultRoute() {
        ...
        if (!def.subButtons?.length)
            return this.normalizeUrl(main);
    ...
    async changeURL(url, forcePush = false, opts = {}) {
        try {
            ...
            try {
                ...
                try {
                    window.history[fallback]({ url: normalized }, '', normalized);
                }
                catch (__) { }
            }
            this._initialNavigation = false;
            ...
            console.error('router.changeURL', err);
        }
    },
    setActiveButtons(main, sub) {
        try {
            ...
            if (subCtr) {
                ...
                // @ts-expect-error TS(2339): Property '_headerV2_buttonManager' does not exist ... Remove this comment to see the full error message
                if (window._headerV2_buttonManager)
                    window._headerV2_buttonManager.state.currentSubButton = found;
            }
        }
        catch (err) {
            console.error('router.setActiveButtons', err);
        }
    },
    ...
    async navigateTo(route, options = {}) {
        ...
        try {
            ...
            let valid = false;
            try {
                valid = await this.validateUrl(normalized);
            }
            catch (_) { }
            if (!valid)
            ...
            const cfg = window._headerV2_buttonManager?.buttonConfig;
            if (!cfg)
                throw new Error('buttonConfig not found');
            const mainButton = (cfg.mainButtons || []).find((b) => b.url === main || b.jsonFile === main);
            if (!mainButton)
                throw new Error('mainButton not found');
            ...
            if (hasSubButtons) {
                ...
                try {
                    ...
                    // @ts-expect-error TS(2339): Property '_headerV2_contentLoadingManager' does no... Remove this comment to see the full error message
                    try {
                        window._headerV2_contentLoadingManager?._updateTopVar();
                    }
                    catch (_) { }
                    this.setActiveButtons(main, chosenSub?.url || chosenSub?.jsonFile || sub);
                }
                catch (e) {
                    console.warn('render sub buttons', e);
                }
            }
            else {
                // @ts-expect-error TS(2339): Property '_headerV2_subNavManager' does not exist ... Remove this comment to see the full error message
    ...
    activateUiOnly(main, sub) {
        try {
            ...
            console.error('router.activateUiOnly', e);
        }
    },
