...
export async function init() {
    ...
    if (typeof window !== 'undefined')
        window._headerV2_bootstrapping = true;
    ...
    try {
        ...
        if (!window._headerV2_router)
            window._headerV2_router = router;
    ...
    // @ts-expect-error TS(2339): Property '_headerV2_contentLoadingManager' does no... Remove this comment to see the full error message
    try {
        window._headerV2_contentLoadingManager && window._headerV2_contentLoadingManager.show();
    }
    catch { }
    ...
    try {
        ...
        window.addEventListener('popstate', async () => {
            try {
                ...
                if (!navMgr)
                    throw new Error('navigationManager missing');
                if (!url || url === '?') {
        ...
        try {
            ...
            // @ts-expect-error TS(2339): Property '_headerV2_router' does not exist on type... Remove this comment to see the full error message
            try {
                window._headerV2_router && window._headerV2_router.markInitialNavigationHandled && window._headerV2_router.markInitialNavigationHandled();
            }
            catch (e) { }
        }
        catch (e) {
