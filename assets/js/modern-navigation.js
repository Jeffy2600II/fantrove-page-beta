...
(function () {
    ...
    const SUPPORTS_BC = (function () { try {
    ...
    const NOW = () => Date.now();
    function safeJSONParse(s, fallback) { try {
    ...
    class StorageAPI {
        static getLS(key) { try {
            return localStorage.getItem(key);
        }
        catch (e) {
            return null;
        } }
        static setLS(key, val) { try {
        ...
        static getSS(key) { try {
            return sessionStorage.getItem(key);
        }
        catch (e) {
            return null;
        } }
        static setSS(key, val) { try {
    ...
    class NavPrefixManager {
        constructor(options = {}) {
            ...
            this.readSelectedLang = options.readSelectedLang || (() => {
                try {
                    return localStorage.getItem('selectedLang') || this.defaultLang;
                }
                catch (e) {
        ...
        buildCandidates(baseUrl) {
            ...
            const langs = [];
            if (parsedCurrent.lang)
                langs.push(parsedCurrent.lang);
            const sel = this.readSelectedLang();
            if (sel && !langs.includes(sel))
                langs.push(sel);
            if (this.defaultLang && !langs.includes(this.defaultLang))
    ...
    class NavRenderer {
        ...
        createFragment(navConfig, currentLang, navPrefixManager) {
            ...
            list.forEach(item => {
                try {
                    ...
                    if (item.url)
                        a.dataset.link = item.url;
                    if (item.go_url)
                        a.dataset.goUrl = item.go_url;
                    ...
                    try {
                        ...
                        lbl.textContent = item.en_label || 'Missing Label';
                    }
                    a.appendChild(lbl);
                    frag.appendChild(a);
                }
        ...
        updateHref(itemEl, href) { try {
            if (itemEl && itemEl.href !== href)
                itemEl.href = href;
        }
        catch (e) { } }
        updateLabel(itemEl, text) { try {
            const l = itemEl.querySelector('.' + this.labelClass);
            if (l)
                l.textContent = text;
        }
        catch (e) { } }
    ...
    class NavController {
        constructor(opts = {}) {
            ...
            // BroadcastChannel
            try {
                this._bc = SUPPORTS_BC ? new BroadcastChannel('fv-lang') : null;
            }
            catch (e) {
                this._bc = null;
            }
            if (this._bc)
                this._bc.onmessage = (ev) => { try {
                    if (ev && ev.data && ev.data.lang)
                        this._onLangChanged(ev.data.lang);
                }
                catch (e) { } };
        ...
        static _isLocalDev() {
            try {
                ...
                if (host.endsWith('.local'))
                    return true;
                if (location.port && ['3000', '5173', '7700', '8080', '3001'].includes(String(location.port)))
                    return true;
        ...
        async init() {
            ...
            try {
                ...
                setTimeout(() => { try {
                    this.navRenderer.reveal(this._navEl);
                }
                catch (e) { } }, 120);
                // start health checks
        ...
        _bind() {
            try {
                ...
                window.addEventListener('languageChange', (e) => { try {
                    if (e?.detail?.language)
                        this._onLangChanged(e.detail.language);
                }
                catch (e) { } });
                ...
                try {
                    this._mo = new MutationObserver(muts => {
                        try {
                            muts.forEach(m => {
                                m.addedNodes.forEach(node => { if (node && node.nodeType === 1)
                                    this._updateLinksIn(node); });
                            });
                        }
                        catch (e) { }
                    });
        ...
        getPredictedLangForPath(path) {
            try {
                ...
                // 1) exact match
                if (map[exact] && map[exact].lang)
                    return map[exact].lang;
                // helper variants
                ...
                function addVariants(p, s) {
                    ...
                    if (v.endsWith('/index.html'))
                        v = v.slice(0, -11) || '/';
                    if (v.endsWith('/home/index.html'))
                        v = v.replace(/\/home\/index\.html$/, '/');
                    if (v !== '/' && v.endsWith('/'))
                        v = v.slice(0, -1);
                    variants.add(v + s);
                    variants.add(v + (s ? s : ''));
                ...
                try {
                    ...
                    if (Array.isArray(recent) && recent.length) {
                        ...
                        for (let i = recent.length - 1; i >= 0; i--) {
                            const rec = recent[i];
                            if (rec && rec.lang && (now - (rec.ts || 0) < this.RECENT_WINDOW_MS)) {
                                return rec.lang;
                            }
                            if (now - (rec.ts || 0) > 120000)
                                break;
                        }
                    }
                }
                ...
                items.forEach(it => {
                    ...
                    try {
                        ...
                        if (cfg) {
                            ...
                            if (newLabel)
                                this.navRenderer.updateLabel(it, newLabel);
                        }
                    }
                    catch (e) { }
                });
            ...
            for (const c of candidates) {
                try {
                    ...
                    if (resp && resp.ok) {
                        ...
                        if (parsed.hasLangPrefix) {
                            try {
                                this._writeStoredLang(parsed.lang);
                            }
                            catch (e) { }
                            try {
                                const rawMap = sessionStorage.getItem('fv-nav-lang-map') || '{}';
                        ...
                        try {
                            ...
                            if (linkEl) {
                                ...
                                if (resolved.search)
                                    urlObj.search = resolved.search;
                                if (resolved.hash)
                                    urlObj.hash = resolved.hash;
                            }
                        }
                        catch (e) { }
                        try {
        ...
        _onPopState(ev) {
            try {
                ...
                if (desiredLang && desiredLang !== stored) {
                    try {
                        this._writeStoredLang(desiredLang);
                    }
                    catch (e) { }
                    // Broadcast + languageChange custom event for in-page listeners
                    try {
                        if (this._bc)
                            this._bc.postMessage({ lang: desiredLang, version: (Number(localStorage.getItem('langVersion') || 0) + 1) });
                    }
                    catch (e) { }
                    try {
                        window.dispatchEvent(new CustomEvent('languageChange', { detail: { language: desiredLang } }));
                    }
                    catch (e) { }
        ...
        _applyScreenBehavior() {
            try {
                ...
                if (isMobile) {
                    this._unmountLeftRail();
                    this._enableMobileSync();
                }
                else {
                    this._mountLeftRail();
                    this._disableMobileSync();
                }
            }
            catch (e) { }
        }
            ...
            try {
                ...
                if (siteMain) {
                    ...
                    for (const c of children)
                        document.body.insertBefore(c, siteMain);
                    if (siteMain.parentNode)
                        siteMain.parentNode.removeChild(siteMain);
                }
                ...
                bodyChildren.forEach(el => { if (el.style && el.style.marginLeft === '88px')
                    el.style.marginLeft = ''; });
            }
            catch (e) { }
        }
        ...
        _disableMobileSync() {
            try {
                ...
                if (this._navEl && !this._externalHidden) {
                    this._navEl.style.transform = 'translateZ(0) translateY(0%)';
                    this._navVisible = true;
                }
        ...
        _hideNav() { if (!this._navEl)
            return; this._navEl.style.transform = 'translateZ(0) translateY(100%)'; this._navVisible = false; }
        _showNav() { if (!this._navEl)
            return; this._navEl.style.transform = 'translateZ(0) translateY(0%)'; this._navVisible = true; }
        // Public API helpers
            ...
            try {
                ...
                this._healthInterval = setInterval(() => {
                    ...
                    if (this._healthChecksRun === 12) {
                        ...
                        this._healthInterval = setInterval(() => { try {
                            this._runHealthCheck();
                        }
                        catch (e) { } }, 30000);
                    }
    ...
    queueMicrotask(() => { controller.init().catch(e => { try {
