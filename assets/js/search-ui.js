...
(function () {
    'use strict';
    // @ts-expect-error TS(2339): Property '__searchUI' does not exist on type 'Wind... Remove this comment to see the full error message
    if (window.__searchUI && window.__searchUI._initialized)
        return;
    // =========================================================
    // CONFIG
    // =========================================================
    ...
    const StorageService = {
        getHistory() { try {
            return JSON.parse(sessionStorage.getItem(CONFIG.STORAGE.historyKey) || '[]');
        }
        catch {
            return [];
        } },
    ...
    const VirtualScrollEngine = {
        ...
        destroy() {
            ...
            if (this._vp && this._onScroll)
                this._vp.removeEventListener('scroll', this._onScroll);
            this._vpObs?.disconnect();
            this._vpObs = null;
            this._sgObs?.disconnect();
            this._sgObs = null;
            this._box?.remove();
            this._box = null;
            this._vis?.clear();
        ...
        _buildOff() {
            ...
            for (let i = 0; i < n; i++)
                this._off[i + 1] = this._off[i] + this._hgt[i];
            this._total = this._off[n] || 0;
        },
        _find(target) {
            if (!this._off || this._off.length < 2)
                return 0;
            let lo = 0, hi = this._off.length - 2;
            while (lo < hi) {
                const mid = (lo + hi + 1) >>> 1;
                if (this._off[mid] <= target)
                    lo = mid;
                else
                    hi = mid - 1;
            }
        ...
        _render() {
            if (!this._vp || !this._box || !this._items.length)
                return;
            ...
            for (const [idx, el] of recycle) {
                ...
                if (this._pool.length < this.POOL_MAX)
                    this._pool.push(el);
                else
                    el.remove();
            }
        ...
        _measure(indices) {
            const exec = () => {
                ...
                if (adj !== 0 && this._vp)
                    this._vp.scrollTop += adj;
            };
            if ('requestIdleCallback' in window)
                requestIdleCallback(exec, { timeout: 500 });
            else
                setTimeout(exec, 50);
        },
    };
    ...
    const KeyboardAutoToggleService = {
        enableAutoToggle(sc) {
            if (State.keyboardAutoToggleEnabled)
                return;
    ...
    const URLService = {
        parseQS(qs) {
            ...
            for (const p of qs.replace(/^\?/, '').split('&')) {
                ...
                if (idx === -1)
                    ...
                    out[decodeURIComponent(p.slice(0, idx))] = decodeURIComponent(p.slice(idx + 1));
            }
            return out;
        },
        buildQS(obj) {
        ...
        commit(state) {
            try {
                if (this.isEqual(state, State.lastCommittedSearchState))
                    return;
    ...
    const RenderingService = {
        ...
        renderResults(results, showSuggestionsIfNoResult = false) {
            try {
                ...
                if (State.overlayOpen && State.scrollableContent) {
                    VirtualScrollEngine.mount(State.scrollableContent, container, filtered, (item, l) => this.renderResultItem(item, l), lang);
                }
                else {
        ...
        _batchRender(items, container, lang) {
            try {
                ...
                console.error('_batchRender failed', e);
            }
        },
        _attachCopyHandler(container) {
            // @ts-expect-error TS(2339): Property '_copyResultTextHandlerSet' does not exis... Remove this comment to see the full error message
    ...
    const ReadyModeService = {
        extractSmartNames() {
            try {
                if (!State.allKeywordsCache)
                    return [];
                const lang = LanguageService.getLang();
                const out = [], seen = new Set();
                for (const kw of State.allKeywordsCache) {
                    if (out.length >= CONFIG.RENDER.suggestionsFullscreenMax)
                        break;
                    if (!kw?.item)
                        continue;
    ...
    const SuggestionService = {
        handleKeydown(ev, container) {
            try {
                ...
                if (ev.key === 'ArrowDown') {
                    ...
                    ev.preventDefault();
                    items[idx === -1 ? items.length - 1 : Math.max(0, idx - 1)]?.focus?.();
                }
        ...
        renderQuerySuggestions(query) {
            try {
                ...
                // @ts-expect-error TS(2339): Property 'SearchEngine' does not exist on type 'Wi... Remove this comment to see the full error message
                ...
                if (inp)
                    inp.onkeydown = e => {
                        ...
                        if (e.key === 'ArrowDown') {
                            e.preventDefault();
                            container.querySelector('.suggestion-item')?.focus?.();
                        }
    ...
    const OverlayService = {
        open() {
            try {
                if (State.overlayOpen || State.overlayTransitioning)
                    return;
            ...
            try {
                ...
                if (inp && headerWrapper) {
                    ...
                    if (iconSpan) {
                        ...
                        if (ref)
                            headerWrapper.insertBefore(inp, ref);
                        else
        ...
        _hideNav() {
            // @ts-expect-error TS(2339): Property 'modernNav' does not exist on type 'Windo... Remove this comment to see the full error message
            try {
        ...
        doSearch(e, preventPush, options) {
            try {
                ...
                if (!q.trim()) {
                    ...
                    if (rc)
                        DOMService.setHTML(rc, placeholder);
                    VirtualScrollEngine.destroy();
                    ...
                    if (State.overlayOpen && options.closeOverlay)
                ...
                try {
                    if (window.SearchEngine?.search)
                        out = window.SearchEngine.search(q, State.selectedType) || out;
                }
                catch { }
        ...
        doSearchFromURL(q, type, category, retryCount) {
            ...
            const attempt = () => {
                try {
                    ...
                    const hasDocs = (() => {
                        try {
                            return (se._internals?.getDocs?.()?.length || 0) > 0;
                        }
                        catch {
                            return false;
                        }
                    ...
                    try {
                        ...
                        try {
                            history.replaceState({ q, type: State.selectedType, category: State.selectedCategory }, '', URLService.buildUrlForState({ q, type: State.selectedType, category: State.selectedCategory }));
                        }
                        catch { }
    ...
    const UIService = {
        ...
        scrollToTop() {
            try {
                if (State.scrollableContent)
                    State.scrollableContent.scrollTop = 0;
            }
            catch { }
        },
        setupFilters() {
            try {
                [CONFIG.DOM.typeFilterId, CONFIG.DOM.categoryFilterId].forEach(id => {
                    ...
                    if (!el)
                        return;
                    const onChange = () => { if (id === CONFIG.DOM.typeFilterId)
                        this.onTypeChange();
                    else
                        this.onCatChange(); };
                    el.onchange = onChange;
                    el.onkeyup = e => { if (e.key === 'Enter')
                        onChange(); };
                });
            }
            catch { }
        ...
        updateUILanguage() {
            try {
                ...
                // @ts-expect-error TS(2339): Property 'placeholder' does not exist on type 'HTM... Remove this comment to see the full error message
                if (inp && inp.placeholder !== ph)
                    inp.placeholder = ph;
                const lbls = DOMService.queryAll('.search-filters-panel .filter-group-label');
                if (lbls.length > 0 && lbls[0].textContent !== LanguageService.t('type'))
                    lbls[0].textContent = LanguageService.t('type');
                if (lbls.length > 1 && lbls[1].textContent !== LanguageService.t('category'))
                    lbls[1].textContent = LanguageService.t('category');
            }
            catch { }
        },
    };
    ...
    function loadData() {
        return _waitForConDataService(CONFIG.TIMING.conDataServiceWaitMs).then(svc => {
            // @ts-expect-error TS(2339): Property 'getAssembled' does not exist on type 'un... Remove this comment to see the full error message
            if (svc)
    ...
    function initializeSearchEngine() {
        try {
            ...
            loadData().then(data => {
                ...
                if (sr)
                    sr.innerHTML = `<div class="search-result-here" style="text-align:center;color:#969ca8;font-size:1.07em;margin-top:30px;">${LanguageService.t('search_result_here')}</div>`;
                UIService.updateUILanguage();
                try {
                    ...
                    if (hs?.q !== undefined)
                        ...
                        if (arr.length) {
                            ...
                            State.lastCommittedSearchState = { q: l.q || '', type: l.type || 'all', category: l.category || 'all' };
                        }
                        else
                ...
                if (init?.q) {
                    ...
                    try {
                        history.replaceState({ q: '', type: 'all', category: 'all' }, '', location.pathname);
                    }
                    catch { }
            ...
            console.error('initializeSearchEngine failed', e);
        }
    }
    ...
    function destroy() {
        try {
            ...
            try {
                ...
                const inp = DOMService.get(CONFIG.DOM.searchInputId);
                if (inp) {
                    if (Handlers.inputInput)
                        inp.removeEventListener('input', Handlers.inputInput);
                    if (Handlers.inputKeydown)
                        inp.removeEventListener('keydown', Handlers.inputKeydown);
                    if (Handlers.inputFocus)
                        inp.removeEventListener('focus', Handlers.inputFocus);
                    if (Handlers.inputClick)
                        inp.removeEventListener('click', Handlers.inputClick);
                }
