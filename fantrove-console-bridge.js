...
(function () {
    ...
    async function flushQueue() {
        if (queue.length === 0 || !isOnline || !isApiHealthy)
            return;
    ...
    function detectCategory(source, level) {
        if (source?.includes('Network') || source?.includes('HTTP'))
            return 'network';
        if (source?.includes('API'))
            return 'api';
        if (source?.includes('Code') || source?.includes('Error'))
            return 'code';
        return 'system';
    }
    function setupErrorCapture() {
        window.addEventListener('error', (event) => {
            if (isNoise(event.message))
                return;
        ...
        window.addEventListener('unhandledrejection', (event) => {
            if (isNoise(String(event.reason)))
                return;
