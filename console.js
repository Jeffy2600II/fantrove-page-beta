...
class FantroveConsolePro {
    ...
    async connectToCloud() {
        ...
        try {
            // Health check
            const healthRes = await this.fetchWithTimeout(`${this.apiUrl}/health`, { method: 'GET' }, 5000);
    ...
    updateActiveUsers(logs) {
        logs.forEach((log) => {
            ...
            if (fp)
                this.activeUsers.add(fp);
        });
        this.lastActiveUsersUpdate = Date.now();
    }
    async loadOlderLogs() {
        if (this.isLoadingHistory || !this.hasMoreOldLogs || !this.isCloudConnected)
            return;
    ...
    setupSmartErrorCapture() {
        ...
        window.addEventListener('unhandledrejection', (event) => {
            if (this.isNoisePromise(event.reason))
                return;
            this.captureError({
                type: 'promise',
                category: 'code',
        ...
        console.error = (...args) => {
            ...
            if (this.isNoiseMessage(message))
                return;
    ...
    async syncPendingLogs() {
        ...
        try {
            const res = await this.fetchWithTimeout(`${this.apiUrl}/logs/batch`, {
                method: 'POST',
    ...
    updateStats() {
        ...
        if (statCode)
            statCode.textContent = this.stats.code;
        if (statNetwork)
            statNetwork.textContent = this.stats.network;
        if (statActive)
            statActive.textContent = this.activeUsers.size;
    }
